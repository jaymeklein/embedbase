...
class StorageConfig(BaseModel):
    ...
    # DEPRECATED — superseded by per-file ``retention_days`` (1-30) on the upload paths
    # (api/services/documents.py::_expiry). Uploads no longer read this global; it is kept
    # only so an existing config.yaml that sets it still validates. The worker purge sweep
    # runs off each document's ``expires_at``, which the per-file retention now stamps.
